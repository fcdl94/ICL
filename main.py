...
parser.add_argument('--mem_size', default=1000, type=int, help='Target mem size')
# method variables
...
for run in range(args.from_run, args.to_run):
    ...
    method = conf.get_method(args.method, da_method=args.da, config=args.config_file, network=network, n_classes=config['n_classes'],
                             n_base=n_base, n_incr=n_incr, features=config['n_features'], target_mem_size=args.mem_size,
